...
module.exports = {
  ...
  updateVagrantBoxName: function (contents, boxName) {
    const regex = /(vagrant_box_name\s+=\s+)(['"]).+?(['"])/gi;
    return contents.replace(regex, `$1$2${boxName}$3`);
  },

  addVagrantHostNames: function (contents, hostNames) {
    const regex = /(config[.]hostmanager[.]aliases\s+=\s+%W)[(](.+?)[)]/gi;
    const hostNamesInFile = regex.exec(contents)[2].split(" ");
    const hostNamesToAdd = hostNames.filter(x => !hostNamesInFile.includes(x));

    return contents.replace(regex, `$1($2${hostNamesToAdd.reduce((acc, v) => `${acc} ${v}`, "")})`);
  },

  updateVagrantAddBoxCommand: function (contents, boxName) {
    const regex = /(vagrant\s+box\s+add\s+)(.+?)(\s+.+)/gi;
    return contents.replace(regex, `$1${boxName}$3`);
  }
}